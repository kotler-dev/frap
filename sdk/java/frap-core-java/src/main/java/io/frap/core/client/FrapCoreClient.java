...
public class FrapCoreClient implements AutoCloseable {
    private static final Logger logger = LoggerFactory.getLogger(FrapCoreClient.class);
     ...
     */
    public static FrapCoreClient create() throws IOException {
        ...
        return new FrapCoreClient(binaryPath);
    }
     ...
     * @throws IOException if the binary cannot be started
     */
    public FrapCoreClient(String binaryPath) throws IOException {
    ...
    /**
     * Checks if the client is still connected to the RPC process.
     */
    ...
    /**
     * Returns the process ID of the RPC process, or -1 if not available.
     */
    public long pid() {
        try {
            return process.pid();
