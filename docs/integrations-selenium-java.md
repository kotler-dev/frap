...
3. `frap.conf` / env: `FLETTA_MIN_CONFIDENCE=0.85`, `FLETTA_REPORT_DIR=build/frap-reports`.
...
| pytest-playwright | Аналог `@frap/frap-playwright` (позже) |
...
Банк с параллельным Playwright-пилотом: `@frap/frap-playwright` — тот же Core, те же отчёты. Java и TS команды сходятся в **одном** формате `frap-events.jsonl` / junit properties.
