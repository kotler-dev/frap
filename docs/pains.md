# Боли и границы ценности frap

Единый источник формулировок **проблем**, которые решает frap, и **границ** — что остаётся на стороне вашего test framework. Использовать для сайта, презентаций, конференций, InSourceHub и sales enablement.
...
frap — **слой вокруг UI-структуры и селекторов**, не замена test lifecycle.

| В зоне frap | Вне зоны frap (ваш фреймворк / команда) |
|---------------|-------------------------------------------|
...
> frap снимает рутину вокруг **селекторов и анализа DOM** — не весь test lifecycle (данные, API, окружения, оркестрация, ваши абстракции).

---

## Универсальные боли (все сегменты)

| ID | Боль | Как проявляется | Что даёт frap | Статус |
|----|------|-----------------|-----------------|--------|
...
**One-liner сегмента:** Дай LLM структурированный доступ к UI — frap даёт агенту надёжные руки и глаза.

---

## Боли, которые frap сознательно не закрывает

Использовать как **anti-pitch** — чтобы не разочаровать ожидания.

| Ожидание | Реальность |
|----------|------------|
| «Напишите все e2e за нас» | frap даёт структуру и стабильность; сценарии — ваша команда или LLM снаружи |
...
| Боль (ID) | Возможность frap | Демо / кейс |
|-----------|-------------------|-------------|
...
**Подзаголовок:** frap — не новый test framework
...
*Мост:* frap — structure engine и explainable resolution поверх вашего раннера.

---

### Конференция — hook (30 сек, устно)

> Сколько часов ваша команда потратила в прошлом квартале не на новые фичи, а на то, чтобы **найти правильный селектор** после очередного рефакторинга? frap не пишет тесты за вас — она даёт **структурированную карту UI**, **устойчивые идентификаторы** и **объяснимый healing**, который можно положить в JUnit XML и показать ИБ. Playwright и Selenium остаются; меняется только слой вокруг DOM.
...
frap is a **structure and locator layer** — not a test runner. It does not replace your data setup, API tests, environments, or orchestration.
