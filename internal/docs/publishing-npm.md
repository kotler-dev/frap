...
Scoped names like `frapcode` require an **npm organization** (or a user account literally named `Frap`).
...
Required capability: publish packages under `@Frap/*`.
...
Order in CI: `frapcode` first, then `frapcode-playwright` (depends on published SDK version).

## Verify

```bash
npm view frapcode version
npm view frapcode-playwright version
```
