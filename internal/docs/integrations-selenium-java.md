...
| pytest-playwright | Аналог `frapcode-playwright` (позже) |
...
Банк с параллельным Playwright-пилотом: `frapcode-playwright` — тот же Core, те же отчёты. Java и TS команды сходятся в **одном** формате `frap-events.jsonl` / junit properties.
