...
- [x] `frapcode`: `HealingEngine` вызывает WASM (`FRAP_TS_FALLBACK=1` для dev)
...
- [ ] Бинарь/подкоманда `frapcode-core rpc` (stdin/stdout JSON-RPC)
...
- **Готово когда**: `cargo test -p frapcode-core` (или имя crate), юнит-тесты heal на фикстурах DOM.
...
typedef struct FrapHealingResult {
    bool healed;
    const char* selector;
    double confidence;
    const char* diff;
...
void frap_free_result(FrapHealingResult* result);
```
...
./frapcode-core rpc < test_input.json
