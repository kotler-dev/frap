...
│  @fletta/playwright │ JUnit Extension │ pytest plugin       │
└────────────────────────────┬────────────────────────────────┘
                             │
┌────────────────────────────▼────────────────────────────────┐
│  Language SDK (thin)                                           │
...
| `discover(url \| context)` | → element map (CLI или in-process) |
| `emitEvent(event)` | healing attempt / resolution / safe-fail → JSONL |
| `writeReports()` | junit / allure / fletta-debug (где применимо) |
...
|------|--------|-----------------|
| **SDK** | `@fletta/sdk-java` | FFI, типы, config, events |
| **Adapter** | `fletta-junit5`, `fletta-selenium` | JUnit Extension, `WebDriver` wrapper, Allure/JUnit export |
| **CLI** | `fletta` | discover/analyze без тестового раннера |

Один Java-проект банка обычно подключает: **SDK + `fletta-junit5`** (и опционально thin wrapper над `WebDriver`).
...
| 5 | Унификация report schema | Один `fletta-events.jsonl` / JUnit properties во всех SDK |
