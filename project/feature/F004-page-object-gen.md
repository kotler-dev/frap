...
2. fletta анализирует структуру страницы
...
```typescript
// Generated by fletta from http://demo-store.local/catalog
export class CatalogPage {
  // Element: product cards (cluster of 12)
  getProductCards() {
    return page.locator('fletta:product-card');
  }

  // Element: category filter
  applyFilter(category: string) {
    return page.locator('fletta:category-filter').selectOption(category);
  }

  // Element: pagination
  goToPage(n: number) {
    return page.locator(`fletta:pagination-page[${n}]`).click();
  }
}
```
